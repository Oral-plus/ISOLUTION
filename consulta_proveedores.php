<?php
/**
 * ISOLUCIÓN API Client para Proveedores
 * Funciones para interactuar con la API de ISOLUCIÓN - Módulo Proveedores
 * 
 * @version 1.0
 */
...
/**
 * Crea un nuevo proveedor en ISOLUCIÓN
 * 
 * @param array $proveedorData Datos del proveedor
 * @return array Resultado de la operación
 */
function createSupplier($proveedorData) {
    global $username, $password, $apiKey, $baseApiUrl;
    
    $url = $baseApiUrl . '/Proveedores/';
    $jsonData = json_encode($proveedorData);
    
    // Registrar la solicitud para depuración
    logApiCall('CREATE_SUPPLIER', $proveedorData, null);
    
    $result = postApiDataWithCurl($url, $jsonData, $username, $password, $apiKey);
    
    // Registrar la respuesta
    logApiCall('CREATE_SUPPLIER_RESPONSE', $proveedorData, $result);
    
    return $result;
}

/**
 * Actualiza un proveedor existente en ISOLUCIÓN
 * 
 * @param array $proveedorData Datos del proveedor a actualizar
 * @return array Resultado de la operación
 */
function updateSupplier($proveedorData) {
    global $username, $password, $apiKey, $baseApiUrl;
...
/**
 * Obtiene la lista de proveedores desde ISOLUCIÓN
 * 
...
/**
 * Obtiene un proveedor específico por su documento
 * 
 * @param string $numeroIdentificacion Número de identificación del proveedor
 * @return array Resultado de la operación
 */
function getSupplierByDocument($numeroIdentificacion) {
    global $username, $password, $apiKey, $baseApiUrl;
    
    $url = $baseApiUrl . '/Proveedores/' . urlencode($numeroIdentificacion);
...
/**
 * Valida los datos mínimos requeridos para un proveedor
 * 
 * @param array $proveedorData Datos del proveedor
 * @return array Resultado de la validación
 */
function validateSupplierData($proveedorData) {
    $requiredFields = ['TipoIdentificacion', 'NumeroIdentificacion', 'Nombre'];
    $missingFields = [];
    
    foreach ($requiredFields as $field) {
        if (empty($proveedorData[$field])) {
            $missingFields[] = $field;
        }
    }
...
/**
 * Crea un proveedor de prueba para verificar la API
 * 
 * @return array Resultado de la operación
 */
function createTestSupplier() {
    $testSupplier = [
        ...
        'NumeroIdentificacion' => '900' . rand(100000, 999999) . '-' . rand(1, 9),
        'Nombre' => 'Proveedor de Prueba API ' . date('Y-m-d H:i:s'),
        'Contacto' => 'Juan Pérez',
        'Telefono' => '300' . rand(1000000, 9999999),
        'Email' => 'test' . rand(1000, 9999) . '@proveedorprueba.com',
    ...
    return createSupplier($testSupplier);
}

// Si se ejecuta directamente este archivo, realizar pruebas
if (basename(__FILE__) == basename($_SERVER['SCRIPT_NAME'])) {
    echo "=== ISOLUCIÓN Proveedores API Test ===\n\n";
    ...
    if ($connectionTest['success']) {
        echo "Testing supplier creation...\n";
        $testResult = createTestSupplier();
